...
#nullable disable
    ;
    #nullable restore
    public partial class QuestionTemplateNode : global::Microsoft.AspNetCore.Components.ComponentBase
    #nullable disable
    {
...
#nullable restore
#line 25 "D:\Code\CloudberrySoftwareSolutions\SST-Law\ContractBuilder-main\SST\SST\Client\Shared\QuestionTemplateNode.razor"
    ...
    public List<ContractQuestionTemplate> selectedItems { get; set; } = new List<ContractQuestionTemplate>();
    ...
    {
        var questionDataFields = await ContractQuestionTemplateService.GetAllAsync("ID", new List<VezaVIGridFilter>()
        {
    ...
    public List<VezaMultiItem> SelectedItems {
        get {
            return selectedItems.OrderBy(x => x.SequenceNumber).Select(x => new VezaMultiItem() { Key = x.ID, DisplayText = TreeBuilder.GetAllTemplates().FirstOrDefault(y => y.ID == x.ContractTransactionTemplateID)?.Name }).ToList();
        }
    }

    public async void TrySelect(VezaMultiItem item)
    {
        ContractQuestionTemplate field = new ContractQuestionTemplate()
        {
            QuestionID = QuestionID,
            ContractTransactionTemplateID = item.Key
        };
        /*SAve*/
        var result = await ContractQuestionTemplateService.AddAsync(field);
    ...
    {
        /*Delete*/
        var result = await ContractQuestionTemplateService.DeleteAsync(item.Key);
        if (result.Successful)
        {
    ...
    {
        _isOpen = true;
        DisplayItems = TreeBuilder.GetAllTemplates().Select(x => new VezaMultiItem() { Key = x.ID, DisplayText = x.Name }).ToList().Where(x => x.DisplayText.Contains(SelectedText, StringComparison.InvariantCultureIgnoreCase)).ToList();
    }
...
#nullable restore
#line 1 "D:\Code\CloudberrySoftwareSolutions\SST-Law\ContractBuilder-main\SST\SST\Client\Shared\QuestionTemplateNode.razor"
        IVezaDataService<ContractQuestionTemplate>
...
#nullable restore
#line 1 "D:\Code\CloudberrySoftwareSolutions\SST-Law\ContractBuilder-main\SST\SST\Client\Shared\QuestionTemplateNode.razor"
                                                   ContractQuestionTemplateService
